...
class WebscrapeSiteMelegetHu(models.Model):
    _inherit = 'webscrape.site'

    @api.model
    def _selection_scraper(self):
        selection = super(WebscrapeSiteMelegetHu, self)._selection_scraper()
        selection.append(('webscrape.scraper.meleget.hu', 'meleget.hu'))
        return selection
...
class WebscrapeScraperMelegetHu(models.Model):
    ...
    def create(self, *args, **kwargs):
        res = super(WebscrapeScraperMelegetHu, self).create(*args, **kwargs)
        res._set_scraper_on_site()
        return res

    @api.depends('site_id')
    def _set_scraper_on_site(self):
        for record in self:
            if not record.site_id.scraper_id:
                record.scraper_is_set = False
                record.site_id.scraper_id = record
